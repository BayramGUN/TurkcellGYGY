...
namespace Movies.Forms
{
    public partial class FormMovies : Form
    {
        ...
        private void updateMoviesPlayerButton_Click(object sender, EventArgs e)
        {
            var movieId = (int)comboBoxMovies.SelectedValue;
            var playerId = (int)comboBoxPlayers.SelectedValue;
            var updatePlayerId = textBoxUpdatePlayerId.SelectedValue;
            var playerRole = textBoxPlayerRole.Text;

            var updateMoviesPlayerRequest = new UpdateMoviesPlayerRequest
            {
                PlayerId = updatePlayerId,
                Role = playerRole
            };
            await movieService.UpdateMoviesPlayer(updateMoviesPlayerRequest, movieId, playerId);
